...
class LoginController extends Controller
{
    public function login(Request $request){                    // метод "Вход пользователя"
        if(Auth::check() || Auth::viaRemember()){               // проверка: если юзер зарегистрирован.. (в т.ч. через куки и токен)
            ...
            return redirect()->intended('/admin');  // переход на страницу до редиректа, а если такой страницы нет - то на страницу admin
        }
        ...
        return redirect(route('user.login'))->withErrors([ // если попытка аутентификации провалилась - редирект и вывод сообщения об ошибке
    ...
    public function logout(Request $request) {
        ...
        return redirect(route('user.login'))->withErrors( // если попытка выхода провалилась - редирект и вывод сообщения об ошибке
            'LogoutError!!!');
    }
